...
<div class="main">
    <slot name="top"></slot>
    <div class="video">
      <video :id="'cutVideo'" class="video-js vjs-big-play-centered"></video>
    </div>
      <textarea ref="text" rows="7" cols="40" class="text" placeholder="Please input">{{inputName.text}}</textarea>
    <div class="mainBox" >
<!--        <div>-->
<!--            类型：{{ type }}-->
...
<!--        </div>-->
        <div>
            <button class="mainBtn ctrlBtn" @click="recOpen">打开录音</button>
            <button class="mainBtn ctrlBtn" @click="recStart">开始录音</button>
            <button class="mainBtn ctrlBtn" @click="recStop">结束录音</button>
            <button class="mainBtn ctrlBtn" @click="saveVideo">替换</button>
        </div>
    </div>

    <div class="mainBox">
        <div style="height:100px;width:180px;border:1px solid #ccc;box-sizing: border-box;display:inline-block;vertical-align:bottom" class="ctrlProcessWave"></div>
        <div style="height:40px;width:180px;display:inline-block;background:#999;position:relative;vertical-align:bottom">
            <div class="ctrlProcessX" style="height:40px;background:#0B1;position:absolute;" :style="{width:powerLevel+'%'}"></div>
            <div class="ctrlProcessT" style="padding-left:10px; line-height:40px; position: relative;">{{ duration+"/"+powerLevel }}</div>
        </div>
    </div>
            ...
            <div v-for="obj in logs" :key="obj.idx">
                <div :style="{color:obj.color==1?'red':obj.color==2?'green':obj.color}">
                    ...
                    <template v-if="obj.res">
                        ...
                        <button @click="recdown64(obj.idx)">保存</button>
...
export default {
  ...
  ,beforeDestroy () {
        this.$video('cutVideo').dispose()
  }
  ,methods:{

    //将新的视频地址以及视频的序号传给父组件
        saveVideo(){
          this.result.cutUrl=replacedV.data.replacedUrl
          this.result.index=this.inputName.index
          console.log(this.result)
          this.$emit('child-event',this.result)
        },
        ...
        ,recdown64:function(idx){
            ...
            var path=this.inputName.cutUrl;
            console.log(path);
            reader.onloadend = function() {
                o.down64Val=reader.result;
                this.temp=o.down64Val
                this.replacement={
                  audioBase64: encodeURIComponent(this.temp.toString()),
                  videoPath: path
                }
              axios.post('http://localhost:8181/download',this.replacement).then(function (resp) {
                console.log(resp)
                replacedV=resp
              })
            };
    ...
    initVideo() {
      //初始化视频方法 循环列表获取每个视频的id
          let myPlayer = this.$video('cutVideo', {
